...
// router로 url의 있는 post의 id 가져오기, useSWR로 개별 post의 data 가져오기
...
export default function CommunityPostDetail() {
  ...
  const { data, mutate, error } = useSWR<PostDetailResponse>(router.query.id ? `/api/posts/${router.query.id}` : null); //post 정보 가져오기 "GET"
  ...
  return (
    <Layout canGoBack hasTabBar>
      <div>
        <span
          className="inline-flex my-3 ml-4 items-center px-2.5 py-0.5 
      rounded-full text-xs font-medium bg-gray-100 text-gray-800"
          ...
          <div className="flex px-4 space-x-5 mt-3 text-gray-700 py-2.5 border-t border-b  w-full">
            <button
              ...
              className={cls(
                ...
                data?.isWondering ? "text-green-600 font-semibold" : ""
              )}
            >
              <svg
                className="w-4 h-4"
            ...
            <div key={answer.id} className="flex items-start space-x-3">
              ...
              <div>
                ...
                <span className="text-xs text-gray-500 block ">
                  {formatDistanceToNow(new Date(answer?.createAt), { addSuffix: true })}
                </span>
        ...
        {/* 댓글 작성하기 */}
        <form onSubmit={handleSubmit(onValid)} className="px-4 space-y-2">
          <TextArea
